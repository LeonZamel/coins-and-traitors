...
class PermEquivariantModel(TorchModelV2, nn.Module):
    """
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, **kwargs):
        ...
        # For the map inputs we do some convolution and pooling along the W dimensions to reduce the size of the model (e.g. WxW = 7*7 vs wxw = 3*3 -> ~80% size reduction)
        # However, we still keep this reduced w dim until later. This makes sure that all combinations of inputs along all dimensions are possible

        ### MAP GENERAL ###
        self.map_general_C_W_W__X_w_w = nn.Sequential(
            nn.Conv2d(map_channels, self.hidden_channels, 3, padding=1),
            nn.MaxPool2d(3,2),
        )
        
        ### MAP AGENTS
        self.map_agents_first_1_P_W_W__X_P_w_w = nn.Sequential(
            nn.Conv3d(1, self.hidden_channels, (1,3,3), padding=(0,1,1)),
            nn.MaxPool3d((1,3,3),(1,2,2)),
        )
        self.map_agents_second_1_P_W_W__X_P_w_w = nn.Sequential(
            nn.Conv3d(1, self.hidden_channels, (1,3,3), padding=(0,1,1)),
            nn.MaxPool3d((1,3,3),(1,2,2)),
        )
        ...
        self.moves_net = nn.Sequential(
            ...
            nn.MaxPool2d(
                3,
                1),
            nn.Flatten(), # The size is 1x1 by this point, we just get rid of these extra channels
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        # shape (batchsize, channels, viewsize, viewsize)
        ...
        x = self.activation(x)
        # shape (batch, self.hidden_channels, num_agents, num_agents, reduced_world_size, reduced_world_size)
        ...
        # shape (batch, 4 * self.hidden_channels, numagents, reduced_world_size, reduced_world_size)
        ...
        # shape (batch, 4 * self.hidden_channels, reduced_world_size, reduced_world_size)
